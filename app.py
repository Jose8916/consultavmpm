...
def search():
    ...
    if request.method == 'POST':
        ...
        cur.execute('SELECT dni, nombres, apellidos, licencia, fecha_revalidacion, categoria FROM tabla_consulta WHERE dni = %s', (dni,))
        result = cur.fetchone()
        cur.close()
        conn.close()
...
def send_verification_email(email, code):
    ...
    msg['From'] = 'desarrolladorantabamba@gmail.com'
    msg['To'] = email

    with smtplib.SMTP('smtp.gmail.com', 587) as server:
        server.starttls()
        server.login('desarrolladorantabamba@gmail.com', 'rnsz dxrg xfel svhn')
...
def forgot_password():
    ...
    send_verification_email('desarrolladorantabamba@gmail.com', verification_code)
    flash('Se ha enviado una clave de 7 caracteres de numeros al correo desarrolladorantabamba@gmail.com.', 'info')
    print('Se ha enviado una clave de 7 caracteres de numeros al correo desarrolladorantabamba@gmail.com.', 'info')
...
def manejodatos():
    ...
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute('SELECT * FROM tabla_consulta ORDER BY dni LIMIT %s OFFSET %s', (per_page, offset))
    data = cur.fetchall()
    cur.execute('SELECT COUNT(*) FROM tabla_consulta')
...
def add_data():
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('INSERT INTO tabla_consulta (dni, nombres, apellidos, licencia, fecha_revalidacion, categoria) VALUES (%s, %s, %s, %s, %s, %s)',
...
def edit_data():
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('UPDATE tabla_consulta SET dni=%s, nombres=%s, apellidos=%s, licencia=%s, fecha_revalidacion=%s, categoria=%s WHERE dni=%s',
...
def delete_data(dni):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('DELETE FROM tabla_consulta WHERE dni = %s', (dni,))
        conn.commit()
        cur.close()
        conn.close()
